...
impl<'a> Builder<'a> {
    ...
    pub fn build(self) -> std::io::Result<Counter> {
        ...
        // Our `perf_event_open_sys::bindings::perf_event_attr` struct is
        // generated by running bindgen against a particular version of the
        // Linux kernel user-space headers. This may not be the same version of
        // the kernel we're actually running on. But based on the kernel
        // comments for `copy_struct_from_user` in `include/linux/uaccess.h`, I
        // think we can pass any version of the struct to any version of the
        // kernel, and things will Just Work.
        //
        // Details:
        //
        // As the API evolves, Linux's practice is to extend the struct by
        // adding new fields to the end. As long as the new fields are zeroed
        // out, the system call is supposed the same effect as when the struct
        // was smaller. In other words, values for the new fields that request
        // new behaviors need to be non-zero values.
        //
        // When we actually pass a struct to `perf_event_open`, there are two
        // interesting cases:
        //
        // -  If we pass a smaller struct than the kernel is expecting, the kernel
        //    just pads it with zeroes out to the expected size, so the effect is
        //    the same as if we'd passed the kernel the size of the struct it
        //    expects.
        //
        // -  If we pass a larger struct than the kernel is expecting, the kernel
        //    checks whether the unexpected bytes are all zero.
        //
        //    -   If they are, the kernel copies in only the bytes it expected.
        //        Since the omitted bytes are all zero, the system call will
        //        have the expected effect.
        //
        //    -   If they are not, it returns E2BIG. So this is what it looks
        //        like to request features the current kernel doesn't support.
        //
        // In every non-error case, the effect is as expected. In every error
        // case, unsupported features were requested.
        //
        // The upshots:
        //
        // -   We don't need to do anything special with the struct size to get
        //     older or newer kernels to accept our attrs structs. For example,
        //     we don't need `Builder` to try to pass the smallest size that
        //     still covers the features requested.
        //
        // -   It's always fine to regenerate `perf_event_open_sys`'s bindings
        //     from the newest kernel sources available, since newer, larger
        //     structs won't confuse older kernels. Only requesting features the
        //     kernel can't support gets an error.
